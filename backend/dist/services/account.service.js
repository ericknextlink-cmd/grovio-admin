...
class AccountService {
  ...
  async deleteAccount(userId, reason) {
    try {
      ...
      if (deletedUserError) {
        console.error(
          "Failed to add to deleted users table:",
          deletedUserError
        );
      ...
      return {
        success: true,
        message:
          "Account deleted successfully. You can recover your account within 30 days if needed.",
      };
    } catch (error) {
  ...
  async initiateAccountRecovery(email) {
    try {
      ...
      return {
        success: true,
        message:
          "Account recovery initiated. Please check your email for recovery instructions.",
      };
    } catch (error) {
  ...
  async completeAccountRecovery(email, recoveryToken, newPassword) {
    try {
      ...
      const tokenResult = await this.tokenService.verifyRecoveryToken(
        email,
        recoveryToken
      );
      if (!tokenResult.success) {
      ...
      // Restore user data
      const { error: restoreError } = await supabase.from("users").insert({
        id: authData.user.id,
        email: deletedUser.email,
        first_name: deletedUser.first_name,
        last_name: deletedUser.last_name,
      ...
      return {
        success: true,
        message:
          "Account recovered successfully. Please verify your email address.",
        user: {
          id: authData.user.id,
          email: deletedUser.email,
