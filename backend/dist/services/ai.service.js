...
class AIService {
    ...
    async generateRecommendations(request) {
        try {
            ...
            const recommendations = this.generateBudgetBasedRecommendations(products, budget, familySize, role, preferences);
     ...
     */
    async generateMealSuggestions(request) {
        try {
...
**What would you like help with today?**`;
    ...
    extractBudgetFromMessage(message) {
        const match = message.match(/(\d+[\.,]?\d*)/);
        return match ? parseFloat(match[1].replace(',', '')) : undefined;
    }
    isBudgetQuery(message) {
    ...
    generateRationale(familySize, role, itemCount) {
    ...
    formatRecommendationsAsChat(recommendations, budget) {
...
**Reasoning:** ${recommendations.rationale}`;
    ...
    formatProductSearchAsChat(products, query) {
...
**What would you like help with?**`;
    ...
    getDefaultMealSuggestions(mealType, familySize) {
        const defaultMeals = [
            {
                name: 'Rice and Stew',
