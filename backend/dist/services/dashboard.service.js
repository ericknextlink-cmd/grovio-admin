...
class DashboardService {
    ...
    async getDashboardStats() {
        try {
            const [productStats, categoryStats, orderStats, transactionStats, revenueStats, growthStats] = await Promise.all([
    ...
    async getRecentActivities(limit = 10) {
        try {
            ...
            const userIds = (recentOrders || []).map((o) => o.user_id).filter(Boolean);
            let usersMap = {};
            if (userIds.length > 0) {
    ...
    async getCategoryStats() {
        ...
        if (categoryData && categoryData.length > 0) {
            ...
            topCategory = Object.keys(categoryCount).reduce((a, b) => categoryCount[a] > categoryCount[b] ? a : b);
        }
        return {
