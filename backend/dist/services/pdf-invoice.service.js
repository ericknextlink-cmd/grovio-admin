...
class PDFInvoiceService {
    ...
    async getTemplatePageSize(templateBytes) {
        const doc = await pdf_lib_1.PDFDocument.load(templateBytes);
    ...
    async generateInvoiceToBuffer(data) {
        ...
        try {
            ...
            const M1 = Math.min(totalItems, this.computeItemsThatFit(data, 0, pageHeight1, ITEM_FLOOR_Y_PAGE1, ITEMS_START_OFFSET_PAGE1));
     ...
     */
    computeItemsThatFitBlankPage(data, startIndex, pageHeight = 842) {
        const items = data.items.slice(startIndex);
        const floorY = 50;
     ...
     */
    async fillTemplate1FirstPage(data, qrCodeDataUrl, template1Bytes, pageHeight, referencePageHeight, itemLimitOverride) {
    ...
    async drawBlankTablePage(data, itemOffset, itemLimit, pageSize) {
        ...
        if (pageSize) {
            ...
            for (const item of items) {
                ...
                page.drawText(item.quantity.toString(), { x: width - 840, y: rowY, size: 38, font: helveticaBold, color: (0, pdf_lib_1.rgb)(0, 0, 0) });
                page.drawText(item.unitPrice.toFixed(2), { x: width - 560, y: rowY, size: 38, font: helveticaBold, color: (0, pdf_lib_1.rgb)(0, 0, 0) });
                page.drawText(item.total.toFixed(2), { x: width - 280, y: rowY + 5, size: 38, font: helveticaBold, color: (0, pdf_lib_1.rgb)(0, 0, 0) });
                page.drawLine({
                    start: { x: lineLeft, y: rowContentBottom },
                    end: { x: lineRight + 90, y: rowContentBottom },
                    thickness: lineThickness,
