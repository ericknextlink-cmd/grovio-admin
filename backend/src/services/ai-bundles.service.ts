 ...
 * Autonomously generates curated product combinations
 * using database products and AI intelligence
 */

export type BundleSource = 'ai' | 'admin'
...
Note: discountPercentage 0 means bundle price = sum of items. Return ONLY the JSON array.`
   ...
   */
  private async generateDeterministicBundles(count: number): Promise<{
    success: boolean
    bundles?: ProductBundle[]
    error?: string
  }> {
  ...
  private async createBundle(
    bundleData: { productIds: string[]; title?: string; description?: string; category?: string; targetAudience?: string; badge?: string; discountPercentage?: number; [key: string]: unknown },
    allProducts: Array<{ id: string; name: string; price: string; [key: string]: unknown }>
  ): Promise<ProductBundle | null> {
    try {
      const bundleProducts = allProducts.filter(p =>
        bundleData.productIds.includes(p.id)
      )
   ...
   */
  async saveBundle(bundle: ProductBundle, generatedBy: BundleSource = 'ai'): Promise<{
    success: boolean
    error?: string
  }> {
    try {
      const { error } = await this.supabase
        ...
        .insert({
          ...
          generated_by: (bundle.generatedBy ?? generatedBy) as string,
          is_active: true,
        })
  ...
  async getBundles(options: {
    ...
    try {
      ...
      const formattedBundles: ProductBundle[] = (bundles ?? []).map((b: { generated_by?: string; [key: string]: unknown }) => ({
        id: String(b.id),
        ...
        productIds: (b.product_ids as string[]) ?? [],
        products: (b.products_snapshot as ProductBundle['products']) ?? [],
   ...
   */
  private matchesBudgetRange(price: number, budgetRange: string): boolean {
    const weekly = price  // Assume bundle is for weekly shopping
    
    if (budgetRange.includes('Under ₵100') && weekly < 100) return true
    if (budgetRange.includes('₵100-200') && weekly >= 100 && weekly <= 200) return true
    if (budgetRange.includes('₵200-500') && weekly >= 200 && weekly <= 500) return true
