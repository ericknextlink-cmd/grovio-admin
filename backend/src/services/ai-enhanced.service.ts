...
export class AIEnhancedService {
  ...
  private categorizeProduct(productName: string): string {
    ...
    if (lowerName.includes('milk') || lowerName.includes('dairy')) return 'Dairy'
    if (lowerName.includes('rice')) return 'Grains & Rice'
    if (lowerName.includes('corn') && lowerName.includes('flake')) return 'Breakfast Cereals'
    if (lowerName.includes('sardine') || lowerName.includes('mackerel') || lowerName.includes('fish')) return 'Canned Fish'
    if (lowerName.includes('oil') || lowerName.includes('palm')) return 'Oils & Fats'
    if (lowerName.includes('spaghetti') || lowerName.includes('pasta')) return 'Pasta & Noodles'
    if (lowerName.includes('sugar')) return 'Sweeteners'
    if (lowerName.includes('biscuit') || lowerName.includes('cracker')) return 'Snacks'
    if (lowerName.includes('seasoning') || lowerName.includes('cube') || lowerName.includes('spice')) return 'Seasonings'
    return 'Other'
  }
  ...
  private extractRecommendedProducts(
    aiResponse: string,
    availableProducts: Array<{ code: string; name: string; unitPrice: number }>
  ): Array<{
    id: string
    name: string
    price: number
    ...
    let match
    const finalListItems = new Set<string>()
    ...
    while ((match = finalListPattern.exec(aiResponse)) !== null) {
      ...
      if (matchedProduct && !finalListItems.has(matchedProduct.code)) {
        ...
        const nearbyText = aiResponse.substring(
  ...
  private findMatchingProduct(
    ...
    availableProducts: Array<{ code: string; name: string; unitPrice: number }>
  ): { code: string; name: string; unitPrice: number } | null {
    const aiLower = aiProductName.toLowerCase()
    
    // First try exact match
    let bestMatch = availableProducts.find(p => 
      p.name.toLowerCase() === aiLower
    )
    
    if (bestMatch) return bestMatch
    
    // Try includes match
    bestMatch = availableProducts.find(p => 
      p.name.toLowerCase().includes(aiLower) || 
    ...
    for (const product of availableProducts) {
      ...
      const matchingWords = aiWords.filter(aiw => 
        productWords.some(pw => pw.includes(aiw) || aiw.includes(pw))
      )
    ...
    try {
      ...
      while (attempts < maxAttempts) {
        try {
          ...
          let systemPrompt = buildSupplierRecommendationPrompt(context, queryIntent, productContext)
   ...
   */
  private smartFilterProducts(
    allProducts: Array<{ code: string; name: string; unitPrice: number; category?: string }>,
    mealType: string,
    budget: number,
    familySize: number
   ...
   */
  private async getConversationHistory(threadId: string, _userId?: string): Promise<ChatMessage[]> {
    try {
   ...
   */
  private async performCulturalDeliberation(
    userRequest: string,
...
SUGGESTIONS: [3 meal ideas using ONLY available products]`
        ...
        const xaiResponse = await this.xaiModel.invoke([
          new SystemMessage('You are an expert in Ghanaian cuisine and West African food culture.'),
          new HumanMessage(xaiPrompt)
        ])
        ...
        const patterns = getMealPatternsForCulture('ghanaian')
    ...
    const validation = validateGhanaianMeal(
      userRequest.toLowerCase().split(/\s+/),
      mealType.toLowerCase() as 'breakfast' | 'lunch' | 'dinner'
    )
...
${productContext.slice(0, 8000)}`
  ...
  async chat(
    ...
    try {
      ...
      const systemPrompt = buildProductRecommendationPrompt(context, productContext)
