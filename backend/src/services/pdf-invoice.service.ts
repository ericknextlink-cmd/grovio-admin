...
const BLANK_PAGE_TEMPLATE_CANDIDATES = ['Template-BLank.pdf', 'Template-blank.pdf', 'template-blank.pdf'] as const
...
export class PDFInvoiceService {
   ...
   */
  private async getTemplateBytes(filename: string = DEFAULT_TEMPLATE_FILENAME): Promise<Uint8Array> {
    ...
    throw new Error(
      `Invoice template not found: ${filename}. Upload templates/${filename} to Supabase bucket "${this.templatesBucketName}" or place in backend/public/`
    )
  }

  /** Load template by primary or alternate filename. Returns null if not found. */
  private async getTemplateBytesOrNull(...filenames: string[]): Promise<Uint8Array | null> {
  ...
  private async getTemplatePageHeight(templateBytes: Uint8Array): Promise<number> {
  ...
  async generateInvoiceToBuffer(data: InvoiceData): Promise<Uint8Array> {
    ...
    try {
      ...
      if (!template1Bytes || !template2Bytes) {
        console.warn(
          'Multi-page invoice: Template-no-footer.pdf or Template.pdf missing. Using single-page Template.pdf.'
        )
        return await this.fillTemplate(data, qrCodeData, templateBytes)
      }
  ...
  private static readonly DESC_LINE_HEIGHT = 42
  private static readonly ITEM_LINE_VERTICAL_OFFSET = 75

  /** Row height for one item (for fit simulation). */
  private getItemRowHeight(item: { description: string }): number {
   ...
   */
  private computeItemsThatFit(
    data: InvoiceData,
    startIndex: number,
    pageHeight: number,
    floorY: number,
    startYOffset: number = 320
  ): number {
  ...
  private async fillTemplate(
    ...
    options?: { itemOffset?: number; itemLimit?: number; skipTotalsAndFooter?: boolean; referencePageHeight?: number }
  ): Promise<Uint8Array> {
    ...
    try {
      ...
      const scaleBarcodeQr =
        referencePageHeight != null && referencePageHeight > 0 && skipTotalsAndFooter
   ...
   */
  private async fillTemplate2TableOnly(
    data: InvoiceData,
    template2Bytes: Uint8Array,
    itemOffset: number,
    itemLimit: number
  ): Promise<Uint8Array> {
    const items = data.items.slice(itemOffset, itemOffset + itemLimit)
    if (items.length === 0) throw new Error('fillTemplate2TableOnly: no items in slice')

    const pdfDoc = await PDFDocument.load(template2Bytes)
  ...
  private async drawBlankTablePage(
    ...
    blankTemplateBytes?: Uint8Array | null
  ): Promise<Uint8Array> {
    const items = data.items.slice(itemOffset, itemOffset + itemLimit)
    ...
    const pdfDoc = useBlankFile
      ? await PDFDocument.load(blankTemplateBytes!)
      : await PDFDocument.create()
    const page = useBlankFile
      ? pdfDoc.getPages()[0]
      : pdfDoc.addPage(
          pageSize ? [pageSize.width, pageSize.height] : [595, 842]
        )
   ...
   */
  private async fillTemplate2LastPage(
    data: InvoiceData,
    qrCodeDataUrl: string,
    template2Bytes: Uint8Array,
    itemOffset: number
    ...
    page.drawText(`GHC ${data.discount.toFixed(2)}`, { x: width - 360, y: yPos, size: 48, font: helvetica, color: rgb(0, 0, 0) })
    yPos -= 86
    page.drawText('Credits', { x: totalsLabelX, y: yPos, size: 38, font: helvetica, color: rgb(0, 0, 0) })
    page.drawText(`GHC ${data.credits.toFixed(2)}`, { x: width - 360, y: yPos, size: 48, font: helvetica, color: rgb(0, 0, 0) })
    yPos -= 86
    page.drawText('Subtotal', { x: totalsLabelX, y: yPos, size: 38, font: helveticaBold, color: rgb(0, 0, 0) })
    page.drawText(`GHC ${(data.discount + data.credits).toFixed(2)}`, { x: width - 360, y: yPos, size: 48, font: helveticaBold, color: rgb(0, 0, 0) })
    yPos -= 38
    page.drawLine({ start: { x: lineLeft, y: yPos + 92 }, end: { x: lineRight + 90, y: yPos + 92 }, thickness: lineThickness, color: rgb(0, 0, 0) })
    page.drawLine({ start: { x: lineLeft, y: yPos + 188 }, end: { x: lineRight + 90, y: yPos + 188 }, thickness: lineThickness, color: rgb(0, 0, 0) })
    yPos -= totalAmountGap
    page.drawLine({ start: { x: lineLeft, y: yPos }, end: { x: lineRight + 90, y: yPos }, thickness: lineThickness, color: rgb(0, 0, 0) })
    yPos -= totalAmountGap
    page.drawText('Total Amount', { x: totalsLabelX, y: yPos - 80, size: 48, font: helveticaBold, color: rgb(0, 0, 0) })
    page.drawText(`GHC ${data.totalAmount.toFixed(2)}`, { x: width - 380, y: yPos - 80, size: 48, font: helveticaBold, color: rgb(0, 0, 0) })
   ...
   */
  private async fillTemplate3LastPage(
    data: InvoiceData,
    qrCodeDataUrl: string,
    template3Bytes: Uint8Array,
    itemOffset: number
  ): Promise<Uint8Array> {
    const pdfDoc = await PDFDocument.load(template3Bytes)
    const page = pdfDoc.getPages()[0]
    const { width, height } = page.getSize()
    ...
    page.drawText('Total Amount', { x: totalsLabelX, y: yPos - 80, size: 48, font: helveticaBold, color: rgb(0, 0, 0) })
    page.drawText(`GHC ${data.totalAmount.toFixed(2)}`, { x: width - 380, y: yPos - 80, size: 48, font: helveticaBold, color: rgb(0, 0, 0) })
    page.drawText('Thank you for shopping with us!', { x: totalsLabelX - 90, y: yPos - 250, size: 60, font: helveticaBold, color: rgb(0, 0, 0) })
