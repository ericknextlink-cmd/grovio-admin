...
export class ProductsService {
  ...
  async createProduct(productData: Omit<Product, 'id' | 'created_at' | 'updated_at'>): Promise<ServiceResult<Product>> {
    try {
      ...
      const { weight_unit: _wu, ...rest } = productData as Record<string, unknown>
  ...
  async updateProduct(id: string, updates: Partial<Product>): Promise<ServiceResult<Product>> {
    try {
      ...
      if (updates.packaging !== undefined) payload.packaging = updates.packaging
      if (updates.in_stock !== undefined) payload.in_stock = updates.in_stock
      if (updates.rating !== undefined) payload.rating = updates.rating
      if (updates.reviews_count !== undefined) payload.reviews_count = updates.reviews_count
