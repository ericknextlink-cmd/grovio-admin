...
export class VoucherService {
  ...
  async updateVoucher(
    ...
    if (params.code !== undefined) {
      const code = params.code.trim().toUpperCase()
      if (!code) return { error: 'Code cannot be empty' }
      payload.code = code
    }
    let existingType: 'percentage' | 'fixed' | null = null
    if (params.discount_value !== undefined && params.discount_type === undefined) {
      const { data: existing } = await this.supabase
        ...
        .eq('id', id)
        .maybeSingle()
      existingType = (existing?.discount_type as 'percentage' | 'fixed' | undefined) ?? null
    }

    if (params.discount_type !== undefined) payload.discount_type = params.discount_type
    ...
    if (params.image_type !== undefined) payload.image_type = params.image_type
   ...
   */
  async listAssignments(voucherId?: string): Promise<Array<{
    id: string
    voucher_id: string
    user_id: string
    used_at: string | null
   ...
   */
  async revokeAssignment(assignmentId: string): Promise<{ ok: true } | { error: string }> {
