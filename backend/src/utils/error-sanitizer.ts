...
export function sanitizeDatabaseError(error: unknown): string {
  ...
  if (errorCode === '23505' || errorMessage.includes('duplicate key')) {
    ...
    if (errorMessage.includes('email')) return 'This email is already registered'
    if (errorMessage.includes('phone_number')) return 'This phone number is already registered'
    if (errorMessage.includes('order_id')) return 'Order ID already exists'
    if (errorMessage.includes('invoice_number')) return 'Invoice number already exists'
    if (errorMessage.includes('username')) return 'This username is already taken'
...
export function sanitizeError(error: unknown): string {
  ...
  if (dbError !== 'An unexpected error occurred') return dbError
  
  // Try auth error
  const authError = sanitizeAuthError(error)
  if (authError !== 'An unexpected error occurred') return authError
  
  // Try API error
  const apiError = sanitizeApiError(error)
  if (apiError !== 'An unexpected error occurred') return apiError
